...
def confirm_overwrite_mapping(label_to_map, dataset):
  prompt_prefix = '!' * 5 + ' '
  msg = (f'{prompt_prefix} Label "{label_to_map}" from dataset "{dataset}" '
         'already exists in current mapping. Overwrite (y/n) ?') 

  print(msg)

  key_input = ''
  while key_input not in ('y', 'n'):
    key_input = input(msg).lower()

  return key_input == 'y'

def agg_list_or_nan(series):
  # Flatten lists if the series contains lists
  items = []
  for x in series:
