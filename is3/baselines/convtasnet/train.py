...
from is3.utils import merge_dicts
...
def main(conf):
  ...
  train_dataset = ImpulseSeparationScenesDataset(
      ...
      sampling_rate=conf['dataset']['sampling_rate'],
  )
  ...
  # train_subset_size = min(len(train_dataset), 10)  # Use a small subset for debugging
  # val_subset_size = min(len(val_dataset), 10)  # Use a small subset for debugging
  ...
  if conf["dataset"]["sampling_rate"]==44100:
    model = ConvTasNet(
      n_src=2,
      sample_rate=44100,
      causal=True,
