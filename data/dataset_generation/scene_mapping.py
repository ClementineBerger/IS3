"""
Module for data exploration of the scene mapping

TO BE ADDED TO THE PY-SALT PACKAGE
into salt/py-salt/py_salt/scene_mapping.py
"""
import os
import pandas as pd
...
class SceneExplorer(MappingExplorer):
  """SceneExplorer class with tools to explore the scene_mapping of the
  Audible taxonomy
  """
  ...
  def __init__(self,
    ...
    if map_md_file_path is None:
      map_md_file_path = os.path.join(assets_dir_path,
                                      'salt_scene_mapping.tsv')

    if roots_md_file_path is None:
      filename = map_md_file_path.rsplit('/', 1)[1].rsplit('.', 1)[0]
      roots_md_file_path = os.path.join(assets_dir_path,
                                        f'{filename}.json')

    std_col = 'standard_scene'
